...
# - (BỔ SUNG) Lưu toàn bộ kết quả thực nghiệm + siêu tham số + kiến trúc + info data
# =========================================================
...
# ==== (BỔ SUNG) LOG TRAINING CSV + TENSORBOARD ====
...
# (BỔ SUNG) Lưu kết quả & hiện đồ thị, đồng thời SAVE ảnh
...
# ===== (BỔ SUNG) GHI FILE KẾT QUẢ/THỰC NGHIỆM =====
...
# Lưu báo cáo phân loại & ma trận nhầm lẫn & dự đoán
with open(os.path.join(OUT_DIR, "classification_report.txt"), "w", encoding="utf-8") as f:
    f.write(report_txt)
with open(os.path.join(OUT_DIR, "classification_report.json"), "w", encoding="utf-8") as f:
    json.dump(
        classification_report(y_test, y_pred, target_names=classes, digits=4, output_dict=True),
...
# Thông tin môi trường/Data/Siêu tham số/Kiến trúc
exp_info = {
    "timestamp": exp_time,
    ...
    "artifacts": {
        ...
        "classification_report_txt": os.path.join(OUT_DIR, "classification_report.txt"),
        "classification_report_json": os.path.join(OUT_DIR, "classification_report.json"),
        "confusion_matrix_csv": os.path.join(OUT_DIR, "confusion_matrix.csv"),
        "confusion_matrix_png": cm_fig_path,
        "y_test_npy": os.path.join(OUT_DIR, "y_test.npy"),
        "y_pred_npy": os.path.join(OUT_DIR, "y_pred.npy"),
        "test_probs_npy": os.path.join(OUT_DIR, "test_probs.npy"),
        "dataset_npz": os.path.join(OUT_DIR, "dataset_seq.npz"),
    },
    "model_summary": model_summary_str,
    "notes": "Thực nghiệm lưu đầy đủ siêu tham số, cấu hình, kết quả và artifact để tái lập."
}
...
print(" -", os.path.join(OUT_DIR, "classification_report.json"))
print(" -", os.path.join(OUT_DIR, "confusion_matrix.csv"))
print(" -", os.path.join(OUT_DIR, "confusion_matrix.png"))
print(" -", os.path.join(OUT_DIR, "y_test.npy"))
print(" -", os.path.join(OUT_DIR, "y_pred.npy"))
print(" -", os.path.join(OUT_DIR, "test_probs.npy"))
print(" -", os.path.join(OUT_DIR, "history.csv"), "(nếu khả dụng)")
print(" -", os.path.join(OUT_DIR, "dataset_seq.npz"))
